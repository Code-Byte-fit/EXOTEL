...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const bookedRoomNos = reservations
        .filter((reservation) => {
          const checkInConflict =
            reservation.CheckIn < checkout && reservation.CheckOut > checkin;
          const checkInTimeConflict =
            reservation.CheckIn === checkin &&
            reservation.CheckInTime >= checkoutTime;
          const checkOutTimeConflict =
            reservation.CheckOut === checkout &&
            reservation.CheckOutTime <= checkinTime;
          return checkInConflict || checkInTimeConflict || checkOutTimeConflict;
        })
        .map((reservation) => reservation.Rooms.map((room) => room.RoomNo))
        .flat();
