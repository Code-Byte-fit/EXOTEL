...
router.post("/", async (req, res) => {
  try {
    ...
    const guestIdentifier = `${FirstName.trim().toLowerCase()}_${LastName.trim().toLowerCase()}`;
    let guest = await Guests.findOne({ where: { Identifier: guestIdentifier } }); // Check if guest already exists
    if (!guest) {
